...
export default function ImportEventScreen() {
  ...
  const [importedEvent, setImportedEvent] = useState<ImportedEvent | null>(
    null
  );
  ...
  const savePointToDatabase = async (point: ImportedPoint) => {
    ...
    console.log(
      `Photo ${photo.uuid} sauvegardée et liée au point ${pointUuid}`
    );
  };
  ...
  const saveGeometryToDatabase = async (geometry: ImportedGeometry) => {
    const geometryData = {
      ...
      GeoJSON:
        typeof geometry.geoJson === "string"
          ? geometry.geoJson
  ...
  const deleteExistingEventData = async (eventUuid: string) => {
    console.log(
      `Suppression des données existantes pour l'événement ${eventUuid}...`
    );
    ...
    for (const point of existingPoints) {
      ...
      const imagePoints = await getAllWhere<{ Image_ID: string }>(
        db,
        "Image_Point",
        ["Point_ID"],
        [point.UUID]
  ...
  const processEventData = async (data: EventDataMessage) => {
    ...
    try {
      ...
      for (const point of points) {
        setReceiveStatus(
          `Import des points... (${pointCount + 1}/${points.length})`
        );
        ...
        const progress =
          ((pointCount + photoCount) / (points.length + totalPhotos)) * 80;
      ...
      for (const geometry of geometries) {
        setReceiveStatus(
          `Import des géométries... (${geoCount + 1}/${geometries.length})`
        );
          ...
          <View style={styles.overlay}>
            ...
            <View style={styles.overlayBottom}>
              <Text style={styles.instructionText}>
                {Strings.importEvent?.centerQRCode ||
                  "Scannez le QR code pour importer"}
              </Text>
              <TouchableOpacity
            ...
            <>
              ...
              <View style={styles.progressContainer}>
                ...
                <Text style={styles.progressText}>
                  Points: {importStats.points}/{importStats.totalPoints} |
                  Photos: {importStats.photos}/{importStats.totalPhotos}
                </Text>
              </View>
