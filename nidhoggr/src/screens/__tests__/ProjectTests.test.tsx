...
import renderer, { ReactTestRenderer, ReactTestInstance, act } from 'react-test-renderer';
...
jest.mock('expo-image-picker', () => ({
    ...
    launchCameraAsync: jest.fn(() => Promise.resolve({ canceled: false, assets: [{ uri: 'test-photo-uri' }] })),
}));
...
describe('Project Tests - Arrange-Act-Assert', () => {
    ...
    describe('CreatePointScreen', () => {
        ...
        test('Test 2: Validation des champs obligatoires', async () => {
            ...
            const saveButton = touchables.find(t => {
                const text = t.findAllByType(Text).find(txt => txt.props.children === 'Valider');
                return !!text;
            });
            ...
            // Assert
            expect(global.alert).toHaveBeenCalledWith(expect.stringContaining('Veuillez'));
        ...
        test('Test 4: Sélection d\'un équipement', async () => {
            ...
            const editButton = touchables.find(t => {
                const text = t.findAllByType(Text).find(txt => txt.props.children === 'Modifier le repère');
                return !!text;
            });
            ...
            const validateButtonText = editButton!.findAllByType(Text).find(txt => txt.props.children === 'Valider la position');
            expect(validateButtonText).toBeTruthy();

            // Act 2: Simuler le déplacement de la carte (changement de région)
            const map = tree!.root.findByType(require('react-native-maps').default);
            const newRegion = { latitude: 50.1234, longitude: 3.5678, latitudeDelta: 0.01, longitudeDelta: 0.01 };
    ...
    describe('EventListScreen', () => {
        test('Test 8: Affichage de la liste des chantiers', async () => {
            ...
            (Queries.getAll as jest.Mock).mockResolvedValue([
                { UUID: '1', Nom: 'Chantier A', Description: 'Desc A', Date_debut: '2025-01-01', Status: 'OK' }
            ]);
            let tree: ReactTestRenderer | undefined;
        ...
        test('Test 9: Ouverture d\'un événement', async () => {
            // Arrange
            const eventItem = { UUID: '1', Nom: 'Chantier A', Description: 'Desc', Date_debut: '2025', Status: 'OK' };
            ...
            const eventButton = touchables.find(t => {
                const text = t.findAllByType(Text).find(txt => txt.props.children === 'Chantier A');
                return !!text;
            });
            ...
            // Assert
            const navigation = useNavigation();
            expect(navigation.navigate).toHaveBeenCalledWith('Event', expect.objectContaining({
                UUID: '1',
                Nom: 'Chantier A'
            }));
        });
    ...
    describe('CreateEventScreen', () => {
        test('Test 11: Création d\'un nouveau chantier', async () => {
            ...
            const inputs = tree!.root.findAllByType(TextInput);
            const nomInput = inputs.find(i => i.props.placeholder === 'Entrez le nom...');
            const descInput = inputs.find(i => i.props.placeholder === 'Entrez une description...');
    ...
    describe('EventScreen', () => {
        test('Test 13: Affichage du Dashboard', async () => {
            ...
            (useRoute as jest.Mock).mockReturnValue({
                params: { UUID: '123', Nom: 'Event Test', Description: 'Desc', Date_debut: '2025', Status: 'OK' }
            });
            ...
            const titleFound = texts.some((t: ReactTestInstance) => t.props.children === 'Event Test');
    ...
    describe('MapScreen', () => {
        test('Test 14: Affichage des points sur carte', async () => {
            ...
            const markers = tree!.root.findAllByType(require('react-native-maps').Marker);
            expect(markers.length).toBeGreaterThan(0);
        });
        test('Test 15: Interaction Marker', async () => {
            // Arrange
            const mockPoint = { UUID: 'p1', Latitude: 48.0, Longitude: 2.0, Type: 'Poteau', Equipement_quantite: 5 };
            ...
            const navigation = useNavigation();
            expect(navigation.navigate).toHaveBeenCalledWith('AddPoint', expect.objectContaining({
                pointIdParam: 'p1'
            }));
        });
    });
    ...
    describe('PointsScreen', () => {
        ...
        test('Test 17: Navigation vers Simulation', async () => {
            ...
            const simButton = touchables.find(t => {
                const text = t.findAllByType(Text).find(txt => txt.props.children === "Simuler l'itinéraire");
                return !!text;
            });
            ...
            // Assert
            const navigation = useNavigation();
            expect(navigation.navigate).toHaveBeenCalledWith('SimulateScreen', expect.anything());
        });
    });
    ...
    describe('PointPhotosScreen', () => {
        test('Test 18: Prise de photo', async () => {
            ...
            const photoButton = touchables.find(t => {
                const text = t.findAllByType(Text).find(txt => typeof txt.props.children === 'string' && txt.props.children.includes('Prendre une photo'));
                return !!text;
            });
        ...
        test('Test 19: Affichage des photos', async () => {
            // Arrange
            const mockPhotos = [{ UUID: 'ph1', Picture: 'base64data', Picture_name: 'test.jpg' }];
    ...
    describe('SimulateScreen', () => {
        ...
        test('Test 22: Démarrage Simulation', async () => {
            ...
            const startButton = touchables.find(t => {
                const text = t.findAllByType(Text).find(txt => txt.props.children === 'Démarrer le parcours');
                return !!text;
            });
            ...
            const stopButton = tree!.root.findAllByType(TouchableOpacity).find(t => {
                const text = t.findAllByType(Text).find(txt => txt.props.children === 'Pause');
                return !!text;
            });
    ...
    describe('Database Utils', () => {
        ...
        test('Test 23: getAll génère le bon SQL', async () => {
            ...
            expect(mockDb.getAllAsync).toHaveBeenCalledWith('SELECT * FROM TestTable');
        });

        test('Test 24: getAllWhere génère le bon SQL avec clause WHERE', async () => {
            const mockDb = { getAllAsync: jest.fn().mockResolvedValue([]) };
            await RealQueries.getAllWhere(mockDb, 'TestTable', ['col1', 'col2'], ['val1', 'val2']);
            expect(mockDb.getAllAsync).toHaveBeenCalledWith(
                expect.stringContaining('SELECT * FROM TestTable WHERE col1 = ? AND col2 = ?'),
                ['val1', 'val2']
            );
        });
        ...
        test('Test 26: insert génère le bon SQL', async () => {
            ...
            expect(mockDb.runAsync).toHaveBeenCalledWith(
                expect.stringMatching(/INSERT INTO TestTable \(col1,col2\) VALUES \(\?,\?\)/),
                ['val1', 123]
            );
        });
